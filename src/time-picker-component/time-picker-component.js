      ...
      <div class="control-group">
        <div class="control"> 
          <select>
            ...
            ${(options || []).map(option => {
              return `<option ${option === "12:00 PM" ? 'selected': ''}>${option}</option>`
            })}
          </select>
        </div>
        <div class="icon"> 
...
export default class TimePickerComponent extends HTMLElement {
  ...
  onSelectChange(e){
    this.dispatchEvent(new CustomEvent(TIMEPICKER_EVENTS.SELECTED_TIME, {detail:e}))
  }
  connectedCallback(){
    const options = getTimes().map(i => new Date(i).toLocaleTimeString(i).replace(':00 ', ' '))
    this.shadowRoot.innerHTML = template({options});
